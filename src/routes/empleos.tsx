  ...
  head: () => ({
    meta: [
      { title: "Vacantes para tu CV — RUMBO" },
      {
        name: "description",
        content:
          "Sube o pega tu CV y RUMBO encuentra vacantes reales ordenadas por coincidencia con tu perfil.",
      },
...
function portalLinks(role: string, where: string) {
  ...
  return [
    ...
    {
      name: "OCC",
      url: `https://www.occ.com.mx/empleos/de-${slug(role)}/${where ? `en-${slug(where)}/` : ""}`,
    },
    {
      name: "Computrabajo",
      url: `https://mx.computrabajo.com/trabajo-de-${slug(role)}${where ? `-en-${slug(where)}` : ""}`,
    },
          ...
          <section className="rounded-2xl bg-surface ring-1 ring-black/5 p-5 space-y-4 h-fit">
            ...
            <label className="block rounded-xl border border-dashed border-line p-4 text-center cursor-pointer hover:bg-background">
              ...
              <div className="text-xs text-muted mt-1">
                {file ? "Clic para cambiar" : "Máx. 10 MB"}
              </div>
            </label>
              ...
              <>
                <div className="rounded-2xl bg-surface ring-1 ring-black/5 p-5">
                  ...
                  <div className="mt-1 text-lg font-semibold">
                    ...
                    <span className="text-muted">{result.profile.seniority}</span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs text-muted font-mono">
                    ...
                    <span>·</span>
                    <span>{[...new Set(result.jobs.map((j) => j.source))].join(" · ")}</span>
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {result.profile.keywords.map((k) => (
                      <span
                        key={k}
                        className="rounded-full bg-accent-soft px-2.5 py-1 font-mono text-[11px]"
                      >
                        {k}
                      </span>
                    ))}
                ...
                {result.jobs.map((j) => (
                  ...
                  >
                    <div
                      className={`grid size-14 shrink-0 place-items-center rounded-xl font-mono text-lg font-semibold ${
                        j.match >= 80
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-line p-4">
              ...
              <button onClick={() => setCvJob(null)} className="text-muted hover:text-ink text-sm">
                Cerrar ✕
              </button>
            </div>
            <div className="flex-1 overflow-auto p-4">
              {cvLoading && (
                <p className="text-sm text-muted animate-pulse">
                  Adaptando tu CV a la vacante… (puede tardar un minuto)
                </p>
              )}
            ...
            {cvOut && (
              <div className="flex justify-end gap-2 border-t border-line p-4">
                <button
                  onClick={() => navigator.clipboard.writeText(cvOut)}
                  className="rounded-lg ring-1 ring-black/10 px-3 py-2 text-sm font-semibold"
                >
                  Copiar
                </button>
                <button
